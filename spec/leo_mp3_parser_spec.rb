  ...
  context 'private methods' do
    describe '.validate_arguments' do
      it 'does not raise an error whith correct arguments' do
        expect { leo_mp3_parser.send(:validate_arguments, language_and_term) }.not_to raise_error
      end
      ...
      it 'raises an error when values are empty' do
        expect { leo_mp3_parser.send(:validate_arguments, language: '', term: '') }.to raise_error(ArgumentError, 'Values can not be emtpy')
      end
      it 'raises an error unless language is ende, esde or frde' do
        expect { leo_mp3_parser.send(:validate_arguments, language: 'wrong_language_selection', term: 'hello') }.to raise_error(ArgumentError, 'Language must be set to ende, esde or frde')
      end
    end
    describe '.compose_query' do
      it 'retuns correct default url' do
        expect(leo_mp3_parser.send(:compose_query, language_and_term)).to eq(correct_url)
      end

      describe '.instantiate_nokogiri_object' do
        it 'response with 200 for correct url' do
          leo_mp3_parser.send(:instantiate_nokogiri_object, correct_url).map do |response|
            expect(response).to eq(200)
          end
        end
        it 'raises an exception when url is false' do
          expect { leo_mp3_parser.send(:instantiate_nokogiri_object, false_url) }.to raise_error(StandardError, 'Nokogiri throws an exception')
        end
      end
      ...
      describe '.compose_url' do
        it 'composes correct final url' do
          expect(leo_mp3_parser.send(:compose_url, correct_url)).to eq('https://dict.leo.org/media/audio/iEUdzIwxBbMpDuh0x2yajA.mp3')
        end
      end
    end
  end
